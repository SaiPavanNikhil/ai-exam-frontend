...
logout(): void {
  ...
  loadAllInterviews(): void {
    ...
    this.http.get<any>(`${environment.apiBaseUrl}/api/interviews`).subscribe({
      next: (res) => {
  ...
  confirmDeleteInterview(interviewId: string): void {
    ...
    this.http.delete<any>(`${environment.apiBaseUrl}/api/interviews/${interviewId}`).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  scheduleInterview(): void {
    ...
    this.http.post<any>(`${environment.apiBaseUrl}/api/schedule-interview`, payload).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  private uploadCandidateFiles(candidateId: number): void {
    ...
    uploads.forEach(({ file, type }) => {
      ...
      fd.append('file', file);
      this.http.post(`${environment.apiBaseUrl}/api/candidates/${candidateId}/upload?file_type=${type}`, fd)
  ...
  detectFace(imageFile: File): void {
    ...
    this.http.post<any>(`${environment.apiBaseUrl}/detect-face`, formData).subscribe({
      next: (res) => {
