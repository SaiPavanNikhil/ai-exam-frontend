...
export class ExamComponent implements OnInit, OnDestroy {
  ...
  startInterview() {
    ...
    if (this.mode === 'self') {
      if (!this.interviewId) {
        this.interviewId = `SELF-PRACTICE-${Date.now()}`;
      }
    ...
    if (this.mode === 'scheduled') {
      const candidateId = this.scheduledCandidateId;
      const course = this.scheduledInterview?.course;
      ...
      this.http.get<any>(
        ...
        next: async (res) => {
          console.log('📚 Scheduled Questions Loaded', res);
          this.interviewId = res.interview_id;
        ...
        error: (err) => {
          ...
          alert(err?.error?.detail || 'Unable to load scheduled questions.');
        }
      });
  ...
  saveAnswer() {
    ...
    const formData = new FormData();
    formData.append('candidate_id', (this.scheduledCandidateId || this.candidateId).toString());
    formData.append('question_id', this.currentQuestion.id.toString());
    formData.append('answer_text', (this.answer || '').trim());
    formData.append('panel_id', this.scheduledInterview.panel_id.toString());

    console.log('💾 Submitting Answer For AI Grading');

    this.http.post<any>(`${environment.apiBaseUrl}/api/submit-and-grade`, formData)
      .subscribe({
        next: (res) => { console.log('✅ Answer Submitted', res); },
        error: (err) => { console.error('❌ Failed To Submit Answer', err); }
      });
  }

  handleNextQuestion() {
    if (this.interviewCompleted || this.isInterviewFinishing) {
      return;
    }
  ...
  loadNextQuestion() {
    if (!this.interviewQuestions || this.currentQuestionIndex >= this.interviewQuestions.length) {
      console.log(`🏁 ${this.mode === 'self' ? 'Self Assessment' : 'Scheduled Interview'} Completed`);
      this.finishInterview();
      return;
    }

    this.currentQuestion = this.interviewQuestions[this.currentQuestionIndex];
    this.currentQuestionText = this.currentQuestion.question_text;
    this.answer = '';
    this.timeLeft = 50;

    console.log(`📚 Question ${this.currentQuestionIndex + 1}/${this.interviewQuestions.length}`);
  ...
  startVideoRecording() {
    ...
    this.videoRecorder.onstart = () => { console.log("🎥 Interview Recording Started"); };
  ...
  startAudioStreaming() {
    ...
    processor.onaudioprocess = (e) => {
      ...
      if (this.socket?.readyState === WebSocket.OPEN && !this.isSpeaking) {
  ...
  async finishInterview() {
    ...
    if (this.mode === 'scheduled' && this.videoRecorder && this.videoRecorder.state === 'recording') {
  ...
  selectMode(choice: 'self' | 'scheduled') {
    ...
    if (choice === 'self') {
      this.selectedCourse = '';
      this.http.get<any>(`${environment.apiBaseUrl}/api/candidate-courses/${this.candidateEmail}`)
        .subscribe({
          next: (res) => { this.allowedCourses = res.allowed_courses; }
        });
  ...
  saveSelfAssessmentAnswer() {
    ...
    const formData = new FormData();
    formData.append('candidate_id', this.candidateId.toString());
    formData.append('question_id', this.currentQuestion.id.toString());
    formData.append('answer_text', answerText);
    formData.append('assessment_id', this.interviewId);
    formData.append('course', this.selectedCourse);

    console.log('📤 Saving Self Assessment Answer');

    this.http.post<any>(`${environment.apiBaseUrl}/api/self-assessment/submit-answer`, formData)
      .subscribe({
        next: (res) => { console.log('✅ Answer Saved Successfully', res); },
  ...
  generateFinalSelfAssessmentResult() {
    ...
    console.log('📊 Generating Final Self Assessment Result:', this.interviewId);
    ...
    this.http.post<any>(
      ...
      next: (res) => {
        console.log('✅ Final Result Generated', res);
        this.loadFinalSelfAssessmentResult();
      },
      error: (err) => {
        console.error('❌ Failed To Generate Final Result', err);
      }
    });
  }

  loadFinalSelfAssessmentResult() {
    this.http.get<any>(`${environment.apiBaseUrl}/api/self-assessment/result/${this.interviewId}`)
      .subscribe({
        next: (res) => {
          ...
          try {
            ...
            console.warn('⚠️ Failed to parse final response JSON', e);
            this.finalResponse = {
              strengths: [],
              improvements: [],
          ...
          console.log('📊 Parsed Final Response:', this.finalResponse);
        },
        error: (err) => {
          this.isEvaluatingResult = false;
  ...
  checkScheduledInterview() {
    ...
    this.http.get<any>(`${environment.apiBaseUrl}/api/interviews/check-schedule/${this.candidateEmail}`)
      .subscribe({
        next: (res) => {
          this.isCheckingSchedule = false;
          console.log('📅 Schedule Check Response:', res);

          this.scheduledInterviewStatus = res.status;
          this.scheduleMessage = res.message;
          this.scheduledInterview = res.interview || null;
          this.scheduledCandidateId = res.interview?.candidate_id || null;

          console.log('📋 Scheduled Interview:', this.scheduledInterview);
          console.log('🆔 Scheduled Candidate ID:', this.scheduledCandidateId);
          console.log('📚 Scheduled Course:', this.scheduledInterview?.course);
        },
        error: (err) => {
          this.isCheckingSchedule = false;
          console.error('❌ Schedule Check Failed:', err);
          alert(err?.error?.detail || 'Unable to verify scheduled interview.');
        }
      });
  }
  ...
  saveCurrentAnswer() {
    ...
    const payload = {
      candidate_id: this.mode === 'scheduled' ? this.scheduledCandidateId : this.candidateId,
      interview_id: this.interviewId,
      question_id: this.currentQuestion.id,
      answer_text: this.answer || '',
      time_taken: 50 - this.timeLeft
    };

    console.log('💾 Saving Answer', payload);

    return this.http.post<any>(`${environment.apiBaseUrl}/api/submit-and-grade`, payload);
  }

  uploadInterviewRecording() {
    const videoBlob = new Blob(this.fullVideo, { type: 'video/webm' });
    ...
    const formData = new FormData();
    formData.append('file', videoBlob, `${this.interviewId}.webm`);
    ...
    this.http.post<any>(
      ...
      next: (res) => { console.log("✅ Recording Uploaded Successfully", res); },
      error: (err) => { console.error("❌ Recording Upload Failed", err); }
    });
  }
}
